...
class Math extends Base
{
    public function handle($params, Template $template)
    {
        ...
        $number = '-?(?:\d+(?:[,.]\d+)?|pi|π)';
        // What is a number
        $functions_or_vars = '((?:0x[a-fA-F0-9]+)|([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*))';
        $operators = '[,+\/*\^%-]';
        // Allowed math operators
        $regexp = '/^((' . $number . '|' . $functions_or_vars . '|(' . $functions_or_vars . '\s*\((?1)*\)|\((?1)*\)))(?:' . $operators . '(?1))?)+$/';
